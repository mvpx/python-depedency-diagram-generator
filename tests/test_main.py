...
class TestFileScanner:
    ...
    def test_init_custom_excludes(self):
        ...
        scanner = FileScanner(
            exclude_dirs={"custom_dir"}, exclude_files={"custom_file.py"}
        )
...
class TestTextGenerator:
    ...
    def test_generate_simple_entity(self):
        ...
        assert "Text Diagram for TestEntity" in diagram
    ...
    def test_generate_with_dependencies(self):
        ...
        assert "Text Diagram for Entity1" in diagram
        assert "Dependencies:" in diagram
...
@patch("core.FileScanner")
@patch("core.CodeParser")
@patch("core.TextGenerator")
@patch("builtins.open", new_callable=mock_open)
def test_generate_diagram_ascii(
    mock_file, mock_text_generator, mock_parser, mock_scanner
):
    ...
    mock_parser_instance.entities = {
        "TestEntity": Entity("TestEntity", "class", Path("test.py"), 10)
    }
    ...
    # Call the function
    generate_diagram("test_dir", "TestEntity", "text", 1, "output.txt")
...
@patch("core.FileScanner")
@patch("core.CodeParser")
@patch("core.MermaidDiagramGenerator")
@patch("builtins.open", new_callable=mock_open)
def test_generate_diagram_mermaid(
    mock_file, mock_mermaid_generator, mock_parser, mock_scanner
):
...
def test_main_default_args(mock_generate_diagram):
    ...
    mock_generate_diagram.assert_called_once_with(
        "test_dir", "TestEntity", "text", 4, None
    )
...
def test_main_custom_args(mock_generate_diagram):
    ...
    main()
    mock_generate_diagram.assert_called_once_with(
        "test_dir", "TestEntity", "mermaid", 2, "output.txt"
    )


@patch("main.generate_diagram", side_effect=Exception("Test error"))
@patch("sys.argv", ["diagrams", "test_dir", "--entity", "TestEntity"])
@patch("sys.stderr")
